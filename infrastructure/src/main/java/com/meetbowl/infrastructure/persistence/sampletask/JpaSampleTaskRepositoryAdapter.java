...
/**
 * domain port를 JPA로 구현하는 adapter 예시다.
 * Entity 변환은 adapter 경계에서만 수행한다.
 */
@Profile("sample-jpa")
@Repository
public class JpaSampleTaskRepositoryAdapter implements SampleTaskRepositoryPort {
    ...
    public JpaSampleTaskRepositoryAdapter(SpringDataSampleTaskRepository springDataSampleTaskRepository) {
        this.springDataSampleTaskRepository = springDataSampleTaskRepository;
    }

    @Override
    public SampleTask save(SampleTask sampleTask) {
        SampleTaskEntity savedEntity = springDataSampleTaskRepository.save(SampleTaskEntity.from(sampleTask));
