 ...
 *
 * <p>활성 멤버십과 같은 조직의 전 직원 공개 워크스페이스를 질문마다 다시 조회하고, 삭제되지 않았으며 인증 사용자의 조직에 속한 워크스페이스만 AI 검색 범위로 전달한다.
 * 제거된 멤버십이나 다른 조직의 워크스페이스 ID가 과거 요청 문맥에서 재사용되지 않게 하는 권한 경계다.
 */
@Component
public class DefaultChatSharedWorkspaceAccessAdapter implements ChatSharedWorkspaceAccessPort {
    ...
    public Set<UUID> findAccessibleSharedWorkspaceIds(UUID userId, UUID organizationId) {
        Set<UUID> workspaceIds =
                memberRepositoryPort.findActiveByUserId(userId).stream()
        ...
        workspaceRepositoryPort.findOrganizationVisible(organizationId).stream()
                .filter(workspace -> !workspace.isDeleted())
                .map(workspace -> workspace.id())
