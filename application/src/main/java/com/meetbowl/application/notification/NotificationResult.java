 ...
 * 알림 한 건의 조회/발송 결과다.
 *
 * <p>type/resourceType은 도메인 enum을 그대로 노출하지 않고 이름 문자열로 변환해 담는다 — API 계층(app-api)이 domain에 의존하지 않도록
 * 하는 경계이며, 메일 결과 타입과 같은 규칙이다. resourceType/resourceId는 딥링크가 없는 알림이면 함께 null이다.
 */
