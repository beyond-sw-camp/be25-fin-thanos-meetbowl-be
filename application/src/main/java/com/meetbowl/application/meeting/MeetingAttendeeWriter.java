 ...
 *
 * <p>역할 규칙: 주최자는 HOST로 자동 포함하고, {@code attendeeUserIds}는 PARTICIPANT로 저장하며, 그중 검토자로 지정된 사용자는
 * REVIEWER로 저장한다. 검토자는 반드시 참석자(주최자 제외) 중에서 지정해야 한다. 참석자 1명당 역할은 하나이므로 주최자(HOST)는 검토자로 지정할 수
 * 없다(participants에서 제외되어 검증에 걸린다).
 *
...
public class MeetingAttendeeWriter {
    ...
    /** 신규 저장(회의 생성). 주최자(HOST) + 초대 참석자(PARTICIPANT) + 검토자(REVIEWER)를 저장한다. */
    ...
    private List<MeetingAttendee> build(
        ...
        // 주최자 = 회의 호스트. 회의당 1명, 항상 HOST로 포함한다.
