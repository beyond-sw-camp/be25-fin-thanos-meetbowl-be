 ...
 *
 * <p>회의실 행에 비관적 쓰기 잠금을 걸어(존재 검증 포함) 같은 회의실에 대한 동시 요청을 직렬화하고, 잠금이 걸린 상태에서 활성
 * 회의(SCHEDULED/IN_PROGRESS)와 시간대가 겹치는지 검사한다. 호출자(생성/수정 UseCase)의 트랜잭션 안에서 실행되므로 잠금이 검사~저장 구간의 경합을
 * 막는다.
 *
