 ...
 *
 * <p>회의 생성·수정 시 지정된 회의록 검토자(REVIEWER 역할 참석자)를 회의 ID로 조회해, 회의록 생성 흐름이 참조할 수 있게 노출한다. 회의록(Minutes)은
 * {@code reviewerUserId}가 필수이므로, 회의록 생성 측은 이 서비스로 검토자를 얻어 {@code Minutes.createDraft(...)}에 전달한다.
 *
 * <p>경계: 검토자 지정/저장은 회의 생성·수정 UseCase가 담당하고(REVIEWER 역할로 저장), 본 서비스는 그 검토자를 읽어 다른 도메인(회의록)에 넘기는
 * 읽기 전용 연동 지점이다. 검토자 사용자 자체의 유효성은 유저(조직) 도메인 책임이다.
 */
@Service
public class MeetingReviewerService {
     ...
     * Minutes.createDraft(...)}의 {@code reviewerUserId}에 넣으면 된다. 회의 생성 때 지정한 검토자(참석자 중 1명)가 그 값으로 나온다.
     */
