 ...
 *
 * <p><b>구현 담당(개인 워크스페이스/캘린더 모듈):</b> 이 인터페이스를 구현한 {@code @Component}(어댑터)를 등록하면, 회의 생성/수정/취소
 * 시 자동으로 호출된다. 구현체가 아직 없으면(미머지 상태) 회의 모듈이 {@code ObjectProvider.ifAvailable}로 호출을 건너뛰므로, 회의 기능 자체는
 * 영향을 받지 않고 구현체가 들어오는 순간부터 일정 투영이 동작한다. (회의 측은 코드 변경 불필요.)
 */
