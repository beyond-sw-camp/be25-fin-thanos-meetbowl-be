 ...
 *
 * <p>회의실·시간이 바뀌면 생성과 동일하게 회의실 행 비관적 잠금 후 시간대 겹침을 재검사한다. 단, 수정 중인 회의 자신은 겹침 대상에서 제외한다(자기 자신과의
 * 겹침을 충돌로 보지 않기 위함).
 */
@Service
public class UpdateMeetingUseCase {
    ...
    private void syncCalendar(Meeting meeting, List<MeetingAttendee> attendees) {
        // 회의 수정은 같은 meetingId로 다시 투영한다. 개인 워크스페이스 구현체가 있으면 기존 MEETING 일정을 갱신하고, 없으면 생성한다(멱등 upsert).
