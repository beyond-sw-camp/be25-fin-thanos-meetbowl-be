 ...
 * {@code hostUserId}는 인증된 요청자(주최자)를 app-api에서 채워 전달한다. {@code attendeeUserIds}는 초대할 참석자이며, 주최자는
 * 자동으로 HOST 참석자로 포함되므로 목록에 없어도 된다.
 *
 * <p>{@code reviewerUserId}는 회의록 검토자다. 검토자는 반드시 참석자(주최자 포함) 중 한 명이어야 하며, 지정 시 해당 참석자에게 reviewer
 * 플래그가 붙는다(신분과 독립적이라 주최자도 검토자가 될 수 있다). null이면 검토자 없이 생성한다.
 *
