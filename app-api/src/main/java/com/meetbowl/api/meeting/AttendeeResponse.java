...
public record AttendeeResponse(UUID userId, String role, boolean reviewer, String attendanceStatus) {

    public static AttendeeResponse from(AttendeeResult result) {
        return new AttendeeResponse(
                result.userId(), result.role(), result.reviewer(), result.attendanceStatus());
    }
}
