 ...
 *
 * <p>{@code reviewerUserId}는 회의록 검토자(필수)다. 반드시 참석자(주최자 포함) 중 한 명이어야 하며, 해당 참석자에게 reviewer 플래그가
 * 붙는다(신분과 독립적이라 주최자도 검토자가 될 수 있다). 참석자 검색(계열사/부서/팀)은 조직 도메인 ElasticSearch 검색(F5)에서 userId를 받아
 * 채운다.
 */
