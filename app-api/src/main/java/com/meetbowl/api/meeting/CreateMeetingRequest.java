 ...
 * 주최자는 인증 토큰에서 가져오므로 본문에 받지 않으며 자동으로 HOST 참석자가 된다. {@code attendeeUserIds}는 초대할 참석자다. 예정 시작/종료의 선후
 * 검증은 도메인에서 수행한다.
 *
 * <p>{@code reviewerUserId}는 회의록 검토자(선택)다. 지정하면 반드시 {@code attendeeUserIds}(참석자) 중 한 명이어야 하며, 해당
 * 참석자는 REVIEWER 역할로 저장된다. 참석자 검색(계열사/부서/팀)은 조직 도메인 ElasticSearch 검색(F5)에서 userId를 받아 채운다.
 */
