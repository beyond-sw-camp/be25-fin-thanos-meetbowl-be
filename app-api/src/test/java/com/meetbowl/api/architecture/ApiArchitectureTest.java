...
/**
 * app-api는 HTTP 진입 계층이다.
 * Controller는 UseCase만 호출하고 repository/entity에 직접 접근하지 않는다.
 */
@AnalyzeClasses(packages = "com.meetbowl.api", importOptions = ImportOption.DoNotIncludeTests.class)
class ApiArchitectureTest {

    @ArchTest
    static final ArchRule api_should_not_depend_on_domain_or_infrastructure = noClasses()
            .that()
    ...
    @ArchTest
    static final ArchRule controllers_should_not_depend_on_repository_types = classes()
            .that()
            ...
            public void check(JavaClass item, ConditionEvents events) {
                item.getDirectDependenciesFromSelf().stream()
                        ...
                        .forEach(dependency -> events.add(SimpleConditionEvent.violated(
                                item,
                                item.getName() + " depends on repository type " + dependency.getTargetClass().getName()
                        )));
            }
        };
    }
