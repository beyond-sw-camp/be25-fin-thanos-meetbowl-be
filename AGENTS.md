...
- 코드 작업 시 클래스의 역할, 중요한 업무 규칙, 복잡한 처리 흐름을 설명하는 한글 주석을 작성한다.
- 주석은 코드만으로 의도를 파악하기 어려운 분기, 변환, 검증, 저장 경계를 중심으로 작성한다.
- 한 파일에 1~2개만 두는 수준으로 끝내지 말고, 의미 있는 코드 블록마다 필요한 만큼 자세히 남긴다.
- 메서드명이나 변수명을 그대로 반복하는 자명한 주석은 작성하지 않는다.
- 주석은 "무엇"보다 "왜"와 "어떤 전제에서 이렇게 하는지"를 설명해야 한다.
- 주석과 실제 코드 동작이 달라지지 않도록 관련 코드를 변경할 때 주석도 함께 수정한다.

---

## 개인 작업 기록

이 레포에서 AI Agent와 코드 또는 문서를 변경한 경우 아래 개인 작업 기록 파일을 갱신한다.
...
- 이 파일은 개인 작업 기록이며 팀 공용 문서로 취급하지 않는다.
- 실제 변경사항이 발생한 작업만 기록한다.
- 작업 요청 목적과 변경 근거를 작성한다.
- 어떤 파일 또는 패키지에서 무엇을 변경했는지 작성한다.
- 기존 동작이 어떻게 바뀌었는지 구체적으로 작성한다.
- 제외한 작업 범위, 검증 방법과 결과, 남은 작업이 있으면 함께 작성한다.
- 성능 개선 작업은 개선 전후 수치를 함께 기록한다.
- 성능 개선 기록에는 측정 대상, 측정 환경, 사용한 도구 또는 방법, 변경 전 수치, 변경 후 수치, 개선 폭을 작성한다.
- 성능 수치를 직접 측정하지 못한 경우 추정치로 기록하지 않고, 측정하지 못한 이유와 필요한 측정 방법을 작성한다.
- 새로운 기록은 기존 내용을 삭제하지 않고 문서 하단에 누적한다.
- `task/`가 Git ignore 대상이더라도 로컬 작업 기록은 계속 갱신한다.

성능 개선 기록 예시:

```text
측정 대상: 회의 원문 전체 조회 API
측정 환경: local H2, 10,000 sentences, JDK 25
측정 방법: ./gradlew test + repository benchmark helper
변경 전: 평균 420ms
변경 후: 평균 130ms
개선 폭: 평균 응답 시간 69% 감소
변경 내용: meeting_id, sequence_no 복합 인덱스 사용
```
